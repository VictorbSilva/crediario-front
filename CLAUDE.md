...
- PWA is configured (`vite-plugin-pwa`, `registerType: 'prompt'`, `devOptions` off). Security rules for `clients` are written, tested against the emulator (65 rule tests, `npm run test:rules`) and **published to production since 03/09/2026**. CI (`.github/workflows/ci.yml`) runs lint, build and both suites on every push and PR — but it never deploys: `firestore.rules` is the one file in this project whose deploy is manual (`firebase deploy --only firestore:rules`), while Vercel publishes the app by itself on merge.
- The state pattern is set: context + provider + hook in three files (the split exists for `react-refresh/only-export-components`), mirrored from `src/auth/`. **All business decisions live in pure, tested functions under `src/lib/`** — `cliente`, `cpf`, `data`, `dinheiro`, `sync`, `texto` — and components only wire them together.
