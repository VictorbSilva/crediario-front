...
- PWA is configured (`vite-plugin-pwa`, `registerType: 'prompt'`, `devOptions` off). Security rules for `clients`, `sales`, `installments` and `payments` are written and tested against the emulator (99 rule tests, `npm run test:rules`). The `clients` rules are **published to production**; the carnê ones are **not yet deployed** — do that before any code writes to those three collections. CI (`.github/workflows/ci.yml`) runs lint, build and both suites on every push and PR — but it never deploys: `firestore.rules` is the one file in this project whose deploy is manual (`firebase deploy --only firestore:rules`), while Vercel publishes the app by itself on merge.
