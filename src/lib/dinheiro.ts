 ...
 */
type PartesDoValor = { inteiros: string; centavos: string }

/**
 * Decide quem é parte inteira e quem é centavo, a partir de quais separadores
 * aparecem. Devolve `null` quando a forma não é reconhecível — em dinheiro,
 * adivinhar a intenção do usuário é pior do que pedir que ele digite de novo.
 */
function separarValor(limpo: string): PartesDoValor | null {
  ...
  if (ultimoPonto >= 0 && ultimaVirgula >= 0) {
    ...
    const inteiros = limpo.slice(0, corte)

    if (!milharValido(inteiros, milhar)) return null
    return { inteiros: inteiros.replaceAll(milhar, ''), centavos: limpo.slice(corte + 1) }
  }

  // Vírgula sozinha é sempre decimal: em pt-BR ela nunca separa milhar.
  if (ultimaVirgula >= 0) {
    const inteiros = limpo.slice(0, ultimaVirgula)
    if (inteiros.includes(',')) return null
    return { inteiros, centavos: limpo.slice(ultimaVirgula + 1) }
  }

  // Ponto sozinho é ambíguo: "1.234" são mil duzentos e trinta e quatro reais,
  // "12.34" são doze reais e trinta e quatro centavos. O que decide é o tamanho
  // do grupo à direita — três dígitos é milhar, um ou dois é decimal.
  if (ultimoPonto >= 0) {
    if (milharValido(limpo, '.')) return { inteiros: limpo.replaceAll('.', ''), centavos: '' }
    if (/^\d*\.\d{1,2}$/.test(limpo)) {
      return { inteiros: limpo.slice(0, ultimoPonto), centavos: limpo.slice(ultimoPonto + 1) }
    }
    return null
  }

  return { inteiros: limpo, centavos: '' }
}
...
export function parseReaisParaCentavos(entrada: string): number | null {
  ...
  const partes = separarValor(limpo)
  if (!partes) return null

  const inteiros = partes.inteiros === '' ? '0' : partes.inteiros
  const centavos = partes.centavos

  // Mais de duas casas não é arredondado em silêncio.
